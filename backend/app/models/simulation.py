...
class Scenario(Base):
    ...
    # 波次生成计划 — 在指定 tick 动态创建/移除实体
    # [{"tick": 1, "action": "spawn", "object_type_id": "...", "name": "导弹-3", "properties": {...}},
    #  {"tick": 5, "action": "spawn", "object_type_id": "...", "name": "导弹-4", "properties": {...}}]
    spawn_schedule: Mapped[list] = mapped_column(JSON, default=list)

    # 在推演中被移除的实体 ID 列表（用于前端过滤展示）
    removed_instance_ids: Mapped[list] = mapped_column(JSON, default=list)

    # 由 spawn_schedule 动态创建的实例 ID 列表（跨 tick 持久化，reset 时清理）
    spawned_instance_ids: Mapped[list] = mapped_column(JSON, default=list)
