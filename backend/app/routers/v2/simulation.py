...
def create_scenario(
    ...
    scenario = Scenario(
        ...
        loop=body.loop,
        spawn_schedule=body.spawn_schedule,
        removed_instance_ids=body.removed_instance_ids,
        spawned_instance_ids=body.spawned_instance_ids,
...
def start_simulation(ontology_id: str, scenario_id: str, db: Session = Depends(get_db)):
    ...
    db.query(ScenarioTick).filter(ScenarioTick.scenario_id == scenario_id).delete()
    db.query(ScenarioEvent).filter(ScenarioEvent.scenario_id == scenario_id).delete()
    from app.models.v2.object_type import Link, ObjectInstance
    db.query(Link).filter(Link.ontology_id == ontology_id).delete()
    # 清理历史 spawn 创建的实例
    spawn_ids = list(scenario.spawned_instance_ids or [])
    if spawn_ids:
        db.query(ObjectInstance).filter(
            ObjectInstance.ontology_id == ontology_id,
            ObjectInstance.id.in_(spawn_ids)
        ).delete(synchronize_session='fetch')
    scenario.spawned_instance_ids = []

    scenario.current_tick = 0
    scenario.removed_instance_ids = []
...
def _simulate_tick(db, scenario, ontology_id, tick, auto_fire=True):
    ...
    # ── 0. 波次生成 + 过滤已移除实体（在加载参与者之前处理）──
    # 加载所有活跃实体 ID（初始参与者 + 之前波次生成的，排除已摧毁的）
    removed = set(scenario.removed_instance_ids or [])
    base_ids = set(scenario.participant_instance_ids or []) | set(scenario.spawned_instance_ids or [])
    active_ids = set(base_ids) - removed

    for spawn in (scenario.spawn_schedule or []):
        if spawn.get("tick") != tick or spawn.get("action") != "spawn":
            continue
        new_id = str(uuid.uuid4())
        props = dict(spawn.get("properties", {}))
        new_inst = ObjectInstance(
            id=new_id, ontology_id=ontology_id,
            object_type_id=spawn["object_type_id"],
            name_cn=spawn.get("name", "新目标"),
            properties=props, confidence=0.95,
        )
        db.add(new_inst)
        db.flush()
        active_ids.add(new_id)
        # 持久化到 spawned_instance_ids 保持跨 tick 可见
        spawned_list = list(scenario.spawned_instance_ids or [])
        spawned_list.append(new_id)
        scenario.spawned_instance_ids = spawned_list
        events.append({
            "tick": tick, "event_type": "state_change",
            "source_instance_id": new_id,
            "description": f"新目标出现: {new_inst.name_cn}",
            "extra": {"spawn": True, "instance_name": new_inst.name_cn},
        })

    # ── 加载参与者与类型映射（使用 active_ids 支持动态增删）──
    participants = (
        db.query(ObjectInstance)
        .filter(ObjectInstance.id.in_(active_ids))
        .all()
    )
    ...
    # ── 7. 移除已摧毁/消亡实体（持久化，防止下一 tick 复活）──
    for tid in destroyed_threat_ids:
        active_ids.discard(tid)
        # 持久化到 removed_instance_ids，确保后续 tick 不再加载
        removed_list = set(scenario.removed_instance_ids or [])
        removed_list.add(tid)
        scenario.removed_instance_ids = list(removed_list)
        # 删除关联的探测/威胁链接，让图谱干净
        stale_links = db.query(Link).filter(
            Link.ontology_id == ontology_id,
            (Link.source_instance_id == tid) | (Link.target_instance_id == tid),
        ).all()
        for link in stale_links:
            db.delete(link)

    if destroyed_threat_ids:
        db.flush()
        events.append({
            "tick": tick, "event_type": "state_change",
            "description": f"{len(destroyed_threat_ids)} 个目标已被移除",
            "extra": {"removed_ids": list(destroyed_threat_ids)},
        })

    # ── 8. 保存快照（使用 active_ids 过滤已移除的实体）──
    active_participants = [p for p in participants if p.id in active_ids]
    instance_states = _snapshot_instances(db, scenario, active_participants)
    active_links = _snapshot_links(db, ontology_id, scenario, active_ids)
...
def _snapshot_links(db, ontology_id, scenario, participant_ids=None):
    """拍下当前所有活跃的 Link"""
